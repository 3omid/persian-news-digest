...
def summarize_category(category: str, items: list) -> dict:
    ...
    system_prompt = (
        ...
        "هرگز نظر شخصی سیاسی نده، فقط تفاوت لحن/زاویه دید منابع رو توصیف کن. خروجی رو دقیقا به "
        "فرمت JSON زیر بده و هیچ متن اضافه‌ای ننویس:\n"
        '{"summary": "خلاصه کلی ۳-۵ جمله‌ای همه اخبار این دسته", '
...
def periodic_top_news(history_items: list, period_label: str) -> str:
    ...
    system_prompt = (
        ...
        "نه متن کامل)، ۵ تا ۸ مورد که به‌نظر مهم‌ترین/پرتکرارترین موضوعات بودن رو انتخاب کن و "
        "در یک لیست کوتاه فارسی بنویس. اگه موضوعی در چند منبع مختلف تکرار شده، احتمالا مهم‌تره."
    )
